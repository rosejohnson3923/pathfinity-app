-- ================================================================
-- Discovered Live! Career Challenge Database Schema
-- ================================================================
-- This schema implements the complete data model for Career Challenge game mode
-- Integrates with existing dl_ tables for room management and user tracking
-- ================================================================
...
CREATE TABLE IF NOT EXISTS cc_industries (
    ...
    created_by UUID REFERENCES dl_users(id),
...
CREATE TABLE IF NOT EXISTS cc_player_collections (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    player_id UUID NOT NULL, -- References existing user/student table
...
CREATE TABLE IF NOT EXISTS cc_trading_post (
    ...
    -- Trade participants
    offering_player_id UUID NOT NULL,
    receiving_player_id UUID, -- NULL for open trades
...
INSERT INTO cc_industries (code, name, description, icon, challenge_categories)
VALUES (
    'esports',
...
SELECT
    ...
    'Your main sponsor is threatening to withdraw funding due to a player controversy. You need to manage the crisis, retain the sponsor, and maintain team morale.',
    'Management',
    'hard',
    75,
    ARRAY['manager'],
    ARRAY['coach', 'analyst', 'social_media_manager']
FROM cc_industries WHERE code = 'esports'
ON CONFLICT (challenge_code) DO NOTHING;

-- Sample role cards for Esports
