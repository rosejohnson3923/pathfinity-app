-- Career Challenge: Executive Decision Maker Database Schema (CORRECTED)
-- Phase 1: Foundation & Infrastructure
-- Created: 2025-01-15
-- This version properly aligns with existing tables and uses text for player IDs

-- ============================================
-- 1. COMPANY ROOMS TABLE (NEW)
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_cc_company_rooms_active ON cc_company_rooms(is_active);
CREATE INDEX IF NOT EXISTS idx_cc_company_rooms_code ON cc_company_rooms(code);

-- ============================================
-- 2. BUSINESS SCENARIOS TABLE (NEW)
-- ============================================
...
CREATE TABLE IF NOT EXISTS cc_business_scenarios (
    ...
    created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    created_by TEXT, -- Changed from UUID REFERENCES users(id) to TEXT
...
CREATE INDEX IF NOT EXISTS idx_cc_scenarios_company ON cc_business_scenarios(company_room_id);
CREATE INDEX IF NOT EXISTS idx_cc_scenarios_type ON cc_business_scenarios(scenario_type);
CREATE INDEX IF NOT EXISTS idx_cc_scenarios_driver ON cc_business_scenarios(business_driver);

-- ============================================
-- 3. SOLUTION CARDS TABLE (NEW)
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_cc_solutions_scenario ON cc_solution_cards(scenario_id);
CREATE INDEX IF NOT EXISTS idx_cc_solutions_perfect ON cc_solution_cards(scenario_id, is_perfect);

-- ============================================
-- 4. LENS EFFECTS TABLE (NEW)
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_cc_lens_solution ON cc_lens_effects(solution_id);
CREATE INDEX IF NOT EXISTS idx_cc_lens_executive ON cc_lens_effects(executive_role);

-- ============================================
-- 5. LEADERSHIP SCORES TABLE (NEW)
-- ============================================
-- Tracks 6 C's scores for each game round
CREATE TABLE IF NOT EXISTS cc_leadership_scores (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    player_id TEXT NOT NULL, -- Changed from UUID to TEXT to match existing pattern
...
CREATE INDEX IF NOT EXISTS idx_cc_leadership_player ON cc_leadership_scores(player_id);
CREATE INDEX IF NOT EXISTS idx_cc_leadership_session ON cc_leadership_scores(session_id);

-- ============================================
-- 6. PLAYER PROGRESSION TABLE (NEW)
-- ============================================
-- Tracks overall player progress and achievements
CREATE TABLE IF NOT EXISTS cc_player_progression (
    player_id TEXT PRIMARY KEY, -- Changed from UUID to TEXT to match existing pattern
...
CREATE INDEX IF NOT EXISTS idx_cc_progression_xp ON cc_player_progression(total_xp DESC);
CREATE INDEX IF NOT EXISTS idx_cc_progression_level ON cc_player_progression(leadership_level);

-- ============================================
-- 7. SCENARIO-COMPANY MAPPING TABLE (NEW)
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_cc_scenario_map_scenario ON cc_scenario_company_map(scenario_id);
CREATE INDEX IF NOT EXISTS idx_cc_scenario_map_company ON cc_scenario_company_map(company_room_id);

-- ============================================
-- 8. DAILY CHALLENGES TABLE (NEW)
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_cc_daily_date ON cc_daily_challenges(challenge_date DESC);
...
CREATE OR REPLACE FUNCTION calculate_six_cs_average(player_id_param TEXT)
RETURNS TABLE(
...
-- Insert company rooms with proper industry references
...
('MEDICORE', 'MediCore Solutions',
    (SELECT id FROM cc_industries WHERE code = 'HEALTH' OR name ILIKE '%health%' LIMIT 1),
...
('CLOUDPEAK', 'CloudPeak Systems',
    (SELECT id FROM cc_industries WHERE code = 'TECH' OR name ILIKE '%tech%' LIMIT 1),
...
('TRENDFWD', 'TrendForward',
    (SELECT id FROM cc_industries WHERE code = 'RETAIL' OR name ILIKE '%retail%' LIMIT 1),
...
('HORIZON', 'Horizon Financial',
    (SELECT id FROM cc_industries WHERE code = 'FINANCE' OR name ILIKE '%financ%' OR name ILIKE '%bank%' LIMIT 1),
...
('QUICKSERVE', 'QuickServe Global',
    (SELECT id FROM cc_industries WHERE code = 'FOOD' OR name ILIKE '%food%' OR name ILIKE '%restaurant%' LIMIT 1),
...
('SKYCONNECT', 'SkyConnect Airlines',
    (SELECT id FROM cc_industries WHERE code = 'TRANSPORT' OR name ILIKE '%transport%' OR name ILIKE '%airline%' LIMIT 1),
...
('GREENGRID', 'GreenGrid Energy',
    (SELECT id FROM cc_industries WHERE code = 'ENERGY' OR name ILIKE '%energy%' OR name ILIKE '%utility%' LIMIT 1),
...
('NEXTGEN', 'NextGen Mobile',
    (SELECT id FROM cc_industries WHERE code = 'TELECOM' OR name ILIKE '%telecom%' OR name ILIKE '%mobile%' LIMIT 1),
...
('PLAYFORGE', 'PlayForge Entertainment',
    (SELECT id FROM cc_industries WHERE code = 'GAMING' OR name ILIKE '%gaming%' OR name ILIKE '%entertainment%' LIMIT 1),
...
('BUILDRIGHT', 'BuildRight Construction',
    (SELECT id FROM cc_industries WHERE code = 'CONSTRUCT' OR name ILIKE '%construct%' OR name ILIKE '%real estate%' LIMIT 1),
...
-- RLS Policies (using TEXT for player_id)
