...
const azureKeyVaultConfig_1 = require("./azureKeyVaultConfig");
...
const initializeConfig = async () => {
    if (configInitialized)
        return;
    try {
        const openAIConfig = await azureKeyVaultConfig_1.azureKeyVaultConfig.getOpenAIConfig();
...
const createAzureClient = async (modelKey) => {
...
class AzureOpenAIService {
     ...
     */
    async generateBulkTestbedContent(grades, subjects, containers) {
     ...
     */
    async generatePersonalizedContent(studentProfile, contentType, difficulty = 'medium') {
...
Return valid JSON with the content structure.`;
     ...
     */
    async generateUnlimitedAssessments(grade, subject, skillCodes, quantity = 50) {
        const questions = [];
...
Return JSON with questions array containing question_text, type, options, correct_answer, explanation.`;
     ...
     */
    async generateTeacherInsights(studentData, performanceMetrics) {
...
Return detailed JSON with actionable teacher recommendations.`;
     ...
     */
    buildEducationalSystemPrompt(contentType, container) {
...
Always return valid JSON with the exact structure requested.`;
     ...
     */
    buildEducationalPrompt(request) {
...
Return JSON with content array containing all required fields for the ${contentType} type.`;
     ...
     */
    getSkillsForGrade(grade, subject) {
     ...
     */
    async generateWithModel(modelKey, prompt, systemPrompt, options = {}) {
     ...
     */
    getCharactersForContext(context) {
     ...
     */
    async performContentSafetyCheck(content) {
        try {
            ...
            const hasInappropriate = inappropriateKeywords.some(keyword => contentStr.includes(keyword));
            if (hasInappropriate) {
