...
$kvResult = az keyvault create --name $keyVaultName --resource-group $resourceGroup --location $location --output json | ConvertFrom-Json
...
function Add-Secret {
    ...
    if ($currentValue.Length -gt 0) {
        ...
        az keyvault secret set --vault-name $keyVaultName --name $SecretName --value $plainText --output none
...
az keyvault secret set --vault-name $keyVaultName --name "azure-endpoints-config" --value $endpointsConfig --output none
