/**
 * Career Match - Game Summary Screen
 *
 * Shows comprehensive results after completing a Career Match game
 * Displays final rankings, XP earned, accuracy (matches found), and streaks
 *
 * Features:
 * - Final leaderboard with rankings
 * - Personal stats (XP, pairs matched, accuracy, streak)
 * - Celebration animations for winners
 * - Play again or return to arcade options
 */
...
interface CareerMatchGameSummaryProps {
  playerResults: PlayerResult[];
  totalPairs: number;
  userXPEarned: number;
  userPairsMatched: number;
  userMaxStreak: number;
  onPlayAgain: () => void;
  onReturnToArcade: () => void;
}

export const CareerMatchGameSummary: React.FC<CareerMatchGameSummaryProps> = ({
  playerResults,
  totalPairs,
  userXPEarned,
  userPairsMatched,
  ...
  const userAccuracy = Math.round((userPairsMatched / totalPairs) * 100);
      ...
      <div className="relative z-10 max-w-5xl mx-auto">
        ...
        >
          ...
          <div className="glass-game text-center p-6">
            ...
            <div className="glass-text-tertiary text-sm">Match Rate</div>
          </div>

          <div className="glass-game text-center p-6">
            <Grid3x3 className="w-8 h-8 glass-icon-success mx-auto mb-2" />
            <div className="text-3xl font-bold glass-text-primary mb-1">
              {userPairsMatched}/{totalPairs}
            </div>
            <div className="glass-text-tertiary text-sm">Pairs Matched</div>
          </div>
        ...
        >
          ...
          <div className="space-y-3">
            {playerResults.map((player, index) => {
              const playerAccuracy = Math.round((player.pairsMatched / player.totalPairs) * 100);
                      ...
                      <div>
                        ...
                        <div className="glass-text-tertiary text-sm flex items-center gap-3 mt-1">
                          <span className="flex items-center gap-1">
                            ...
                            {player.pairsMatched}/{player.totalPairs} pairs
                          </span>
                          <span className="flex items-center gap-1">
                            <Zap className="w-4 h-4" />
                            {player.maxStreak}x streak
                          </span>
                          <span className="flex items-center gap-1">
...
export default CareerMatchGameSummary;
