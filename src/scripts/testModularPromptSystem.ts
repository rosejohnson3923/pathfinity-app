...
async function testModularSystem() {
  ...
  for (let i = 0; i < TEST_SCENARIOS.length; i++) {
    ...
    try {
      ...
      if (scenario.shouldInclude) {
        const missing = scenario.shouldInclude.filter(term => 
      ...
      if (scenario.shouldNotInclude) {
        const found = scenario.shouldNotInclude.filter(term => 
          questionText.includes(term.toLowerCase())
        );
        if (found.length > 0) {
      ...
      const hasSkillRelevance = skillWords.some(word => 
  ...
  for (let i = 0; i < 3; i++) {
    const result = await perfectPipeline.runCompletePipeline(
      consistencyScenario.user,
      consistencyScenario.system
    );
  ...
  fs.writeFileSync(
    'modular-system-test-report.json',
