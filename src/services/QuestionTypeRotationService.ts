...
class QuestionTypeRotationService {
  private static instance: QuestionTypeRotationService;
  private typeRotation: Map<string, number> = new Map();
  ...
  static getInstance(): QuestionTypeRotationService {
   ...
   */
  getTypesForLearnContainer(
    skillId: string,
    subject: string
  ): { practiceTypes: string[], assessmentType: string } {
   ...
   */
  resetRotation(skillId: string, subject: string) {
    const key = `${subject}-${skillId}`;
   ...
   */
  getRotationStatus(): Map<string, { currentIndex: number, nextType: string }> {
   ...
   */
  forceTypes(types: string[]): string[] {
   ...
   * Get progress report
   */
  getProgressReport(skillId: string, subject: string): {
    tested: string[],
    remaining: string[],
    progress: number
  } {
...
export const questionTypeRotationService = QuestionTypeRotationService.getInstance();